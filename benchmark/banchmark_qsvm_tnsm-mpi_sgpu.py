...
dd = np.zeros((2,2))
run_tnsm(dd, 2)
# A. just follow the previous benchmark data
for mq in range(2,34):
    dd = np.zeros((2,mq))
    run_tnsm(dd,mq)
for mq in range(42,200,10):
    dd = np.zeros((2,mq))
    run_tnsm(dd,mq)
for mq in [200,300,400,500,600,784]:
    dd = np.zeros((2,mq))
    run_tnsm(dd,mq)

# B. single gpu with single data pair for m qubits to test the limitation of qubits
for mq in [2, 4, 8, 16, 32, 64, 128, 256, 512, 784, 1024, 2048, 2352, 3072, 4096]:
    dd = np.zeros((2,mq))
    run_tnsm(dd, mq)