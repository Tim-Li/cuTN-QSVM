...
def kernel_matrix_tnsm_mpi(y_t, x_t, opers, indices_list, network, root, mode=None):
    kernel_matrix = np.zeros((len(y_t),len(x_t)))
    i = -1
    with network as tn:
        for i1, i2 in indices_list:
...
def run_tnsm(n_dim, nb1, nb2):
    ...
    path, info = network.contract_path(optimize={'samples': 8})
    ...
    t0 = time.time()   
    tnsm_kernel_matrix_train_mpi = kernel_matrix_tnsm_mpi(data_train, data_train, oper_train, indices_list_t, network, root, mode=None)
    tnsm_kernel_mpi_t = round((time.time()-t0),3)
    print(n_dim,circuit_t,exp_t,path_t,tnsm_kernel_mpi_t,len(circuit_train))
