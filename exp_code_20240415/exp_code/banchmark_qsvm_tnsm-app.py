...
X,Y = X[c01],Y[c01]
data_train, label_train = X[:1600],Y[:1600]
...
def run_tnsm(n_dim, nb1, nb2):
    ...
    svc.fit(tnsm_kernel_matrix_train,Y_train[:nb1])
    result1 = svc.score(tnsm_kernel_matrix_train,Y_train[:nb1])
    result2 = svc.score(tnsm_kernel_matrix_test,Y_val[:nb2])
    print(n_dim,result1,result2,circuit_t,exp_t,oper_t,path_t,tnsm_kernel_t,len(circuit_train))

run_tnsm(2,10,1)
# run_tnsm(128,1000,100)
# run_tnsm(128,800,100)
for q in range(2,37):
    run_tnsm(q,50,100)
for q in range(40,200,20):
    run_tnsm(q,50,100)
