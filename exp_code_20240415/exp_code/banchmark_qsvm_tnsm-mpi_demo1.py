...
def run_tnsm(n_dim, nb1, nb2):
    ...
    num_qc = len(oper_train)
    chunk, extra = num_qc // size, num_qc % size
    print(num_qc,size,chunk,extra,rank)
    data_begin = rank * chunk + min(rank, extra)
    data_end = num_qc if rank == size - 1 else (rank + 1) * chunk + min(rank + 1, extra)
    print(data_begin,data_end)
    data_index = range(data_begin,data_end)
    oper_train_rank = oper_train[data_begin:data_end]
    indices_list_rank = indices_list_t[data_begin:data_end]
    print(f"Process {rank} is processing data range: {data_index}.",len(oper_train_rank))
       
    # network.autotune(iterations=20)
    path_t = round((time.time()-t0),3)
    t0 = time.time()     
    amp_list = operand_to_amp(oper_train_rank, indices_list_rank, network)
    ...
    if rank == root:
        print(n_dim,circuit_t,exp_t,oper_t,path_t,tnsm_kernel_t,len(oper_train),len(amp_list))
